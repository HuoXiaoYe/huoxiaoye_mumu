...
const iconArr: string[] = ['edit', 'search', 'delete', 'check', 'star']
            ...
            <div className="button-item">
...

                {
                    BUTTONDESC.map((item, index) => {
                        return <ul key={index}>
                            <li style={{lineHeight:'40px'}}>{item}</li>
                            {
                ...
                <ul>
                    <li style={{lineHeight:'40px'}}>图标按钮</li>
                    {
                        buttonList.map((item,index) => {
                            return <li key={item.type}><MUButton size="small" circle={true} type={item.type} icon={iconArr[index]}></MUButton></li>
                        })
                    }
                </ul>
            </div>
